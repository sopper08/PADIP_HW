...
void Widget::exec_spatial_resolution(int val){
    ...
    QImage* spatial_image = new QImage(width*valScale,height*valScale,QImage::Format_RGB32);
    for(int i=0;i<width*valScale;i++){
        for(int j=0;j<height*valScale;j++){
            ...
            spatial_image->setPixel(i,j,qRgb(setPix,setPix,setPix));
        }
    }
    ui->label_GAA_i->setPixmap(QPixmap::fromImage(*spatial_image));
}
...
void Widget::exec_ConBri_func(){
    QImage gaaImg_ConBri = gaaImg;
    QImage gbaImg_ConBri = gbaImg;
